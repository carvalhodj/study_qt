...
int main(int argc, char **argv) {
    ...
    //// Exec directly
    // delete_statement.exec("DELETE FROM contacts WHERE first_name = 'Sara'");
    // qDebug() << "Number of rows affected: " << delete_statement.numRowsAffected();

    //// With prepare()
    // delete_statement.prepare("DELETE FROM contacts WHERE first_name = ?");
    // delete_statement.addBindValue("Sara");
    // delete_statement.exec();
    // qDebug() << "Number of rows affected: " << delete_statement.numRowsAffected();

    // Update contact
    QSqlQuery update_statement(db_conn);
    update_statement.exec("UPDATE contacts SET first_name='Jude' WHERE id=4");
